"""PIDNet-lite for multi-class lane segmentation (ml/README.md step 4).
...
this way to stay self-contained in this codebase and avoid pulling in an
external framework/repo whose training conventions and 19-class Cityscapes
head don't match our setup anyway.

Why PIDNet's design is a good fit here: lane markings are thin,
boundary-dominated structures, and this task is specifically about
distinguishing adjacent classes by their edges (solid vs. dashed, left vs.
right, lane_1 vs. lane_2) -- exactly what PIDNet's three-branch design
targets --
  ...
  - D (boundary) branch: shallow, predicts an auxiliary boundary map (any
    class-to-class transition, not just foreground/background -- see
    losses.extract_boundary), and that boundary map gates how much the
    final prediction trusts the detail branch vs. the context branch at
    each pixel (the "Bag" fusion below) -- exactly where a class boundary
...
class PIDNetLite(nn.Module):
    def __init__(self, in_channels: int = 3, num_classes: int = 4, base_channels: int = 32) -> None:
        ...
        self.boundary_head = nn.Conv2d(c, 1, 1)  # binary: any class-to-class edge, not per-class
        ...
        self.main_head = nn.Conv2d(c * 2, num_classes, 1)
        self.aux_head = nn.Conv2d(c * 4, num_classes, 1)  # deep supervision on i_layer3
